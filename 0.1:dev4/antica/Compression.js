...
// Simplified LZ4.
antica.compress = (s = [] ?? new Uint8Array()) => {
    let d = [];
    let map = {};
    let i = 0;
    let j = 0;
...
antica.decompress = (s = [] ?? new Uint8Array()) => {
    let d = [];
    let i = 0;
    while (i < s.length) {
        let t = s[i++];
...
antica.compressString = s => antica.compress(new TextEncoder().encode(s));
antica.decompressString = s => new TextDecoder().decode(new Uint8Array(antica.decompress(s)));
