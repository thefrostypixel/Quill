...
antica.DOMUtils = class $Element {
    ...
    get children() {
        ...
        this.#elements.forEach(e => [...e.components].forEach(c => children.add(c)));
    ...
    insert(child, index) {
        if (this.#elements.length) {
            ...
            index = index instanceof Array ? [...this.#elements[0].components].findIndex(e => index.includes(e)) : index instanceof Node ? [...this.#elements[0].components].indexOf(index) : index;
            index = index < 0 ? Infinity : index;
            child.forEach(c => this.#elements[0].insertBefore(typeof c == "string" ? document.createTextNode(c) : c, this.#elements[0].components[index++] || null));
        }
        return this;
    }
