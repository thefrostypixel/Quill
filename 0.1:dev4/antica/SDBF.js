...
antica.SDBF = { // Structured Data Binary Format
        ...
        let writeDynamic = (object, binary) => {
            if ([undefined, null, false, true, -1, 0, 1].includes(object)) {
                ...
                if (Number.isInteger(object) && -0x80000001 < object && object < 0x80000000) {
                    ...
                    binary.push(6 + bytes.length, ...bytes);
                } else {
                    let array = new ArrayBuffer(8);
                    new DataView(array).setFloat64(0, object);
                    binary.push(11, ...new Uint8Array(array));
                }
                ...
                if (length < 65) {
                    binary.push(code + length, ...data);
                } else {
                    let bytes = [];
                    do {
                        bytes.push(length & 0xFF);
                    } while (length >>= 8);
                    binary.push(code + 64 + bytes.length, ...bytes, ...data);
                }
            }
            return binary;
        };
        let writeField = (object, field, binary) => {
            ...
            if (field.type == "int") {
                ...
                if (field.content.type == "bool") {
                    ...
                    (field.length ? Array.from({length: field.length}, (_, i) => object[i]) : object).forEach(obj => writeField(obj, field.content, binary));
                }
            } else if (field.type == "map") {
                ...
                if (field.key.type == "bool") {
                    ...
                    keys.forEach(key => writeField(key, field.key, binary));
                }
                if (field.value.type == "bool") {
                    ...
                    values.forEach(value => writeField(value, field.value, binary));
                }
                ...
                if (format.tableGroups[field.table]) {
                    ...
                    if (entry.table) {
                        writeTable(object, namedTableIDs[entry.table], binary);
                    }
                } else {
                    writeTable(object, namedTableIDs[field.table], binary);
                }
            } else if (field.type == "dynamic") {
                writeDynamic(object, binary);
            } else {
        ...
        let writeTable = (object, tableID, binary) => {
            ...
            while (fields.length) {
                ...
                if (field == optionalFields[0] || field.type == "bool" && (!field.optional || object[field.name] != undefined)) {
                    ...
                    writeField(object[field.name], fields.shift(), binaryTable);
                } else {
                    fields.shift();
                }
            }
            writeUVInt(binaryTable.length, binary);
            binary.push(...binaryTable);
        };
        writeField(object, format.main ?? {type: "dynamic"}, binary);
        // console.log(toHex(binary));
        if (format.compression ?? true) {
            binary = antica.compress(binary);
        }
        if (format.encryption ?? true) {
            binary = antica.encrypt(binary, password);
        }
        if (format.id) {
            let id = [];
    ...
    fromBinary: (binary = [] || new Uint8Array(0), format = {}, password = "") => {
        if (format.id && !antica.SDBF.probe(binary, format).matchesFormat) {
            throw new Error("Non-matching format");
        }
        if (format.encryption ?? true) {
            let decrypted = antica.decrypt(format.id ? binary.slice(8) : binary, password);
            if (!decrypted) {
        ...
        if (format.compression ?? true) {
            binary = antica.decompress(binary);
        }
        let i = 0;
        let readUVInt = () => {
            let num = 0;
