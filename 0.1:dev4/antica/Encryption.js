...
antica.encrypt = (data = [], password = "") => {
    data = [...antica.checksum(data), ...data];
    if (password) {
        let nonce = crypto.getRandomValues(new Uint8Array(8));
        nonce[0] |= 1;
        return [...nonce, ...antica.applyEncryptionXor(data, nonce, password)];
    }
    return [0, ...data];
};
antica.decrypt = (data = [], password = "") => {
    let decrypted = data[0] ? antica.applyEncryptionXor(data.slice(8), data.slice(0, 8), password) : data.slice(1);
    let checksum = antica.checksum(data = decrypted.slice(4));
    if (decrypted[0] == checksum[0] && decrypted[1] == checksum[1] && decrypted[2] == checksum[2] && decrypted[3] == checksum[3]) {
        return data;
    }
};
antica.checksum = (data = []) => {
...
antica.applyEncryptionXor = (data = [], nonce = [], password = "") => {
